...
export default function WholesaleForm({ onSubmit, isLoading = false }: WholesaleFormProps) {
  ...
  const backLength = measurements.backLength;
  const lengthSkipMode = backLength === 0;

  const isFormComplete = () =>
    measurements.breed &&
    backLength !== undefined && backLength >= 0 &&
    ...
    (lengthSkipMode || measurements.tailType);

  const validateForm = (): boolean => {
    const newErrors: Record<string, string | undefined> = {};
    if (!measurements.breed) newErrors.breed = "Please select a breed";
    if (backLength === undefined || backLength === null || backLength < 0)
      newErrors.backLength = 'Enter a measurement, or 0 to match on neck & chest only';
    if (!measurements.neckCircumference || measurements.neckCircumference <= 0)
      newErrors.neckCircumference = 'Enter a valid neck measurement';
    if (!measurements.chestCircumference || measurements.chestCircumference <= 0)
      newErrors.chestCircumference = 'Enter a valid chest measurement';
    if (!lengthSkipMode && !measurements.tailType)
      newErrors.tailType = "Please select a tail type";
    setErrors(newErrors);
  ...
  const handleSubmit = () => {
    ...
    onSubmit(
      {
        ...m,
        tailType: lengthSkipMode ? 'straight' : m.tailType,
          ...
          <div className="px-8 py-10">
            ...
            <div className="mb-10">
                  ...
                  <div key={field}>
                    ...
                    {field === 'backLength' && (
                      <p className="text-xs text-gray-400 mt-1">
                        Enter 0 to match on neck &amp; chest only
                      </p>
                    )}
                  </div>
                ))}
              ...
              {lengthSkipMode && (
                <div className="mt-4 flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm text-amber-800">
                  ...
                  Length and tail type excluded — patterns ranked by neck &amp; chest fit only.
                </div>
              )}
            </div>
            ...
            <div className="mb-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Tail type — hidden in length-skip mode */}
                {!lengthSkipMode && (
                  <div>
